...
const std::string Orders::Cols::_ticket_id = "ticket_id";
...
Orders::Orders(const Json::Value &pJson, const std::vector<std::string> &pMasqueradingVector) noexcept(false)
{
    ...
    if(!pMasqueradingVector[2].empty() && pJson.isMember(pMasqueradingVector[2]))
    {
        ...
        {
            ticketId_=std::make_shared<int32_t>((int32_t)pJson[pMasqueradingVector[2]].asInt64());
        }
    }
...
const int32_t &Orders::getValueOfTicketId() const noexcept
{
    static const int32_t defaultValue = int32_t();
    if(ticketId_)
        return *ticketId_;
...
const std::shared_ptr<int32_t> &Orders::getTicketId() const noexcept
{
    return ticketId_;
}
void Orders::setTicketId(const int32_t &pTicketId) noexcept
{
    ticketId_ = std::make_shared<int32_t>(pTicketId);
    dirtyFlag_[2] = true;
}

const int32_t &Orders::getValueOfStatus() const noexcept
{
    static const int32_t defaultValue = int32_t();
    if(status_)
        return *status_;
...
const std::shared_ptr<int32_t> &Orders::getStatus() const noexcept
{
    return status_;
}
void Orders::setStatus(const int32_t &pStatus) noexcept
{
...
const std::string &Orders::getValueOfUserInfo() const noexcept
{
...
const std::shared_ptr<std::string> &Orders::getUserInfo() const noexcept
{
    return userInfo_;
}
void Orders::setUserInfo(const std::string &pUserInfo) noexcept
{
    userInfo_ = std::make_shared<std::string>(pUserInfo);
    dirtyFlag_[5] = true;
}
void Orders::setUserInfo(std::string &&pUserInfo) noexcept
{
    userInfo_ = std::make_shared<std::string>(std::move(pUserInfo));
    dirtyFlag_[5] = true;
}
...
Json::Value Orders::toJson() const
{
    ...
    if(getUserInfo())
    {
        ret["user_info"]=getValueOfUserInfo();
    }
    else
    {
    ...
    const std::vector<std::string> &pMasqueradingVector) const
{
    ...
    if(pMasqueradingVector.size() == 6)
    {
        ...
        if(!pMasqueradingVector[2].empty())
        {
            if(getTicketId())
            {
                ret[pMasqueradingVector[2]]=getValueOfTicketId();
            }
            else
            {
        ...
        if(!pMasqueradingVector[5].empty())
        {
            if(getUserInfo())
            {
                ret[pMasqueradingVector[5]]=getValueOfUserInfo();
            }
            else
            {
...
bool Orders::validateJsonForCreation(const Json::Value &pJson, std::string &err)
{
    ...
    if(pJson.isMember("ticket_id"))
    {
        if(!validJsonOfField(2, "ticket_id", pJson["ticket_id"], err, true))
            return false;
    }
    else
    {
    ...
    if(pJson.isMember("user_info"))
    {
        if(!validJsonOfField(5, "user_info", pJson["user_info"], err, true))
            return false;
    }
    else
    {
...
bool Orders::validateJsonForUpdate(const Json::Value &pJson, std::string &err)
{
    ...
    if(pJson.isMember("ticket_id"))
    {
        if(!validJsonOfField(2, "ticket_id", pJson["ticket_id"], err, false))
            return false;
    }
    if(pJson.isMember("status"))
    {
